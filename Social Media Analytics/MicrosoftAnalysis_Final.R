for (i in c('SnowballC','slam','tm','Matrix','tidytext','dplyr','hunspell','purrr', 'reshape2', 'textstem')){
...
if(!require("plotly")) install.packages("httr"); library("plotly")
if(!require("httr")) install.packages("httr"); library("httr")
library(ggplot2)
library(udpipe)
...
# Remove punctuation and numbers with regular expressions
ht_c_edit <- mutate(ht_c, text = gsub(x = text, pattern = "[0-9]+|[[:punct:]]|\\(.*\\)",
                                      replacement = ""))
...
#create word cloud
#save(ht_cFreq, file = "ht_cFreq.RData")
...
#save(ht_c_editBigramCount, file = "ht_c_editBigramCount.RData")
...
#save(ht_c_summarySentiment_edited, file = "ht_c_summarySentiment_edited.RData")
...
#save(ht_statusSentiment, file = "ht_statusSentiment_3.RData")
...
#save(ht_sentiment, file = "ht_sentiment_1.RData")
...
#save(top_ht_c_retweet, file = "top_ht_c_retweet.RData")
...
#save(top_ht_c_reply, file = "top_ht_c_reply.RData")
...
#save(top_ht_c_like, file = "top_ht_c_like.RData")
...
#save(top_ht_c_quote,file = "top_ht_c_quote.RData")
